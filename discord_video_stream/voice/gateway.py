...
class VoiceGateway:
    ...
    async def connect(
        ...
        *,
        ...
        assert hello["op"] == OP_HELLO, f"Expected OP 8 Hello, got {hello['op']}"
        ...
        assert ready["op"] == OP_READY, f"Expected OP 2 Ready, got {ready['op']}"
        ...
        session_desc = await self._recv()
        assert session_desc["op"] == OP_SESSION_DESCRIPTION, \
            f"Expected OP 4, got {session_desc['op']}"
        ...
        asyncio.create_task(self._receive_loop(), name="voice-recv-loop")
